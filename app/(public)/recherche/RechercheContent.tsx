...
const QUICK = ["Chaussures", "Vêtements", "Électroniques", "Accessoires", "Montre", "Casque", "PC"];
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-white text-[#0F172A] placeholder-gray-400 rounded-xl pl-11 pr-11 py-3.5 text-sm outline-none focus:ring-2 focus:ring-green"
            />
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        {hasQuery && !loading && results.length > 0 && (
          <div className="flex items-center justify-between gap-3 mb-6 flex-wrap">
            <p className="text-[#64748B] text-sm">
              <span className="font-bold text-[#0F172A]">{results.length}</span> résultat{results.length !== 1 ? "s" : ""} pour « {input.trim()} »
            </p>
            <label className="flex items-center gap-2 text-sm">
              ...
              <select
                ...
                className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-sm text-[#0F172A] outline-none focus:border-green"
              >
                {SORTS.map((s) => (
        ...
        {!hasQuery ? (
          ...
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold text-[#0F172A] mb-2">Aucun résultat pour « {input.trim()} »</h2>
            <p className="text-[#64748B] mb-6">Vérifiez l&apos;orthographe ou essayez d&apos;autres mots-clés.</p>
            <div className="flex flex-wrap justify-center gap-4">
              <Link href="/boutique" className="bg-green text-white px-6 py-2.5 rounded-xl font-medium hover:bg-[#15803d] transition-colors">
                Voir toute la boutique
              </Link>
              <Link
                href={getWhatsAppUrl(`Bonjour Odm's Shopping, je cherche "${input.trim()}". Est-ce que vous l'avez ?`)}
                target="_blank"
