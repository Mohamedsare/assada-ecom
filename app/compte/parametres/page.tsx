...
export default function ParametresPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-xl font-bold text-[#0A2A52]">Informations personnelles</h1>

      {/* Profile form */}
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <h2 className="font-semibold text-[#0A2A52] mb-4">Profil</h2>
        <form action={handleProfileSubmit}>
            ...
            {[
              ...
              <div key={name}>
                ...
                <input
                  ...
                  defaultValue={value}
                  readOnly={readOnly}
                  className={`w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm outline-none transition-colors ${readOnly ? "bg-gray-50 text-gray-400 cursor-not-allowed" : "focus:border-[#2F9E44]"}`}
                />
              </div>
            ))}
          </div>
          ...
          <button
            ...
            className="mt-4 flex items-center gap-2 bg-[#2F9E44] text-white px-5 py-2.5 rounded-xl text-sm font-medium btn-sweep hover:bg-[#237A34] transition-colors disabled:opacity-60"
          >
            {savingProfile ? (
