...
export default async function CommandesPage() {
  ...
  return (
    <div>
      <h1 className="text-xl font-bold text-[#0A2A52] mb-6">
      ...
      {orders.length === 0 ? (
        <div className="bg-white rounded-2xl border border-gray-100 p-12 text-center">
          ...
          <p className="text-lg font-semibold text-[#0A2A52] mb-2">Aucune commande</p>
          <p className="text-[#64748B] mb-6">Vous n&apos;avez pas encore passé de commande</p>
          <Link
            href="/boutique"
            className="bg-[#2F9E44] text-white px-6 py-2.5 rounded-xl font-medium btn-sweep hover:bg-[#237A34] transition-colors"
          >
                ...
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-bold text-[#0A2A52]">{order.order_number}</h3>
                      <span
                  ...
                  <p className="font-bold text-[#0A2A52]">{formatPrice(order.total_amount)}</p>
                </div>
