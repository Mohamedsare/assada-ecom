    ...
    <div className="space-y-6">
        ...
        {[
          ...
          return (
            <div key={stat.label} className="bg-white rounded-2xl border border-gray-100 p-4">
              ...
              <p className="text-2xl font-bold text-[#0A2A52]">{stat.value}</p>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5">
        <div className="flex items-center justify-between mb-4">
          <h2 className="font-bold text-[#0A2A52]">Mes dernières commandes</h2>
          <Link href="/compte/commandes" className="text-sm text-[#0A2A52] hover:underline flex items-center gap-1">
        ...
        {recentOrders.length === 0 ? (
          <div className="text-center py-8">
            ...
            <Link href="/boutique" className="text-sm text-[#0A2A52] hover:underline mt-2 inline-block">
              ...
              <div key={order.id} className="flex items-center gap-4 p-3 rounded-xl border border-gray-50 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span className="font-semibold text-sm text-[#0A2A52]">{order.order_number}</span>
                ...
                <div className="text-right shrink-0">
                  <p className="font-bold text-[#0A2A52] text-sm">{formatPrice(order.total_amount)}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-[#0A2A52]">Mes adresses</h2>
            <Link href="/compte/adresses" className="text-sm text-[#0A2A52] hover:underline">
              Gérer
            </Link>
          </div>
          {defaultAddress ? (
            <div className="flex items-start gap-3 p-3 bg-gray-50 rounded-xl">
              <MapPin size={16} className="text-[#0A2A52] mt-0.5 shrink-0" />
              <div className="text-sm">
                <p className="font-medium text-[#0A2A52]">{defaultAddress.full_name}</p>
                <p className="text-[#64748B]">{defaultAddress.district}, {defaultAddress.city}</p>
                <p className="text-[#64748B]">{defaultAddress.phone}</p>
                <span className="text-xs text-[#0A2A52] font-medium">Adresse par défaut</span>
              </div>
            </div>
          ) : (
            <Link
              href="/compte/adresses"
              className="flex items-center gap-2 text-sm text-[#0A2A52] hover:underline p-3 bg-gray-50 rounded-xl"
            >
              <MapPin size={16} />
              Ajouter une adresse
            </Link>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-bold text-[#0A2A52]">Informations personnelles</h2>
            <Link href="/compte/parametres" className="text-sm text-[#0A2A52] hover:underline">
              Modifier
            </Link>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              ...
              <span className="font-medium text-[#0A2A52]">
                {[profile.first_name, profile.last_name].filter(Boolean).join(" ") || "—"}
              </span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#64748B]">Email</span>
              <span className="font-medium text-[#0A2A52] truncate max-w-40">{profile.email ?? "—"}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-[#64748B]">Téléphone</span>
              <span className="font-medium text-[#0A2A52]">{profile.phone ?? "—"}</span>
            </div>
            <div className="flex justify-between">
