...
export default function StocksContent({ products }: { products: Product[] }) {
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold text-[#0F172A]">Gestion des stocks</h1>
...
function StockRow({ product }: { product: Product }) {
  ...
  return (
    <tr className="hover:bg-gray-50 transition-colors">
      ...
      <td className="py-3 px-4"><span className="text-sm font-semibold text-[#0F172A]">{formatPrice(product.current_price)}</span></td>
      <td className="py-3 px-4"><span className="text-sm font-bold text-[#0F172A]">{stock}</span></td>
      <td className="py-3 px-4">
        <div className="flex items-center gap-1.5">
          ...
          {dirty && (
            <button onClick={save} disabled={pending} className="text-xs font-semibold bg-green text-white px-2.5 py-1.5 rounded-lg hover:bg-[#15803d] disabled:opacity-60 transition-colors">
              {pending ? "…" : "OK"}
            </button>
          )}
