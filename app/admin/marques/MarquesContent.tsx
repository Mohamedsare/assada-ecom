  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-xl font-bold text-[#0A2A52]">Marques</h1>
      ...
      {brands.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-100 p-16 text-center">
          ...
          <p className="font-semibold text-[#0A2A52] mb-1">Aucune marque</p>
          ...
          {filtered.map((brand) => {
            ...
            return (
              <div key={brand.id} className="bg-white rounded-lg border border-gray-100 shadow-sm p-5 text-center group">
                ...
                <p className="font-semibold text-[#0A2A52] text-sm">{brand.name}</p>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 sticky top-0 bg-white">
          <h2 className="font-bold text-[#0A2A52]">{isEdit ? "Modifier la marque" : "Nouvelle marque"}</h2>
          <button onClick={onClose} className="text-gray-400 hover:text-[#0A2A52] transition-colors"><X size={20} /></button>
        </div>

        <form action={action} className="p-5 space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#0A2A52] mb-1.5">Nom<span className="text-red-500 ml-0.5">*</span></label>
