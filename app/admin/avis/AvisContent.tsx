...
export default function AvisContent({ reviews }: { reviews: Review[] }) {
  ...
  return (
    <div>
      <div className="mb-5">
        <h1 className="text-xl font-bold text-[#0F172A]">Avis clients</h1>
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex gap-4">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-2">
          <p className="text-sm font-semibold text-[#0F172A] truncate">{review.product?.name ?? "Produit supprimé"}</p>
        ...
        {review.comment && <p className="text-sm text-[#0F172A] mt-2">{review.comment}</p>}

        <div className="flex items-center gap-2 mt-3">
          {!approved ? (
            <button onClick={() => toggle(true)} disabled={pending} className="flex items-center gap-1 text-xs font-semibold bg-green text-white px-3 py-1.5 rounded-lg hover:bg-[#15803d] disabled:opacity-60 transition-colors">
