  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#020B27] flex items-center gap-2">
            <Gift size={22} className="text-[#B8925A]" /> Coffrets cadeaux
          </h1>
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-lg border border-gray-100 p-16 text-center">
          ...
          <p className="font-semibold text-[#020B27] mb-2">{search ? "Aucun coffret ne correspond" : "Aucun coffret"}</p>
          <p className="text-[#64748B] text-sm mb-6">
            {search ? "Essayez un autre terme de recherche." : "Créez votre premier coffret cadeau en regroupant plusieurs produits."}
          </p>
          {!search && (
            <Link href="/admin/coffrets/nouveau" className="bg-[#B8925A] text-white px-5 py-2.5 rounded-lg text-sm font-medium btn-sweep hover:bg-[#9E7A45] transition-colors">
              Nouveau coffret
            </Link>
          )}
        </div>
                    ...
                    <tr key={pack.id} className="hover:bg-gray-50 transition-colors">
                      <td className="py-3 px-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="block text-sm font-medium text-[#020B27]">{pack.name}</span>
                        </div>
                      </td>
                          ...
                          {itemCount === 0 ? (
                            ...
                            <>
                              {(pack.pack_items ?? []).slice(0, 3).map((it) => (
                                <span key={it.id} className="text-[11px] bg-gray-100 text-[#020B27] px-1.5 py-0.5 rounded truncate max-w-[120px]">
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">
                        <p className="text-sm font-bold text-[#020B27] whitespace-nowrap">{formatPrice(pack.current_price)}</p>
                        {pack.old_price && (
                          <p className="text-xs text-gray-400 line-through whitespace-nowrap">{formatPrice(pack.old_price)}</p>
                        )}
                      </td>
                      <td className="py-3 px-4 whitespace-nowrap">
                        <span className={`text-sm font-medium ${pack.stock_quantity === 0 ? "text-red-600" : "text-[#020B27]"}`}>{pack.stock_quantity}</span>
                      </td>
                      <td className="py-3 px-4 whitespace-nowrap">
                      ...
                      <td className="py-3 px-4 whitespace-nowrap">
                        <div className="flex items-center gap-1">
                          <Link href={`/produit/${pack.slug}`} target="_blank" className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#020B27] transition-colors" title="Voir sur le site">
                            <Gift size={15} />
                          </Link>
                          <Link href={`/admin/coffrets/${pack.id}/modifier`} className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-[#020B27] transition-colors" title="Modifier">
                            <Pencil size={15} />
                          </Link>
