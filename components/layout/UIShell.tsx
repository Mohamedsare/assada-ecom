...
function CartDrawerInner({
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 bg-[#020617] text-white shrink-0">
        ...
        {items.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-4 px-6 text-center">
            ...
            <div>
              <p className="font-bold text-[#0F172A] text-lg">Votre panier est vide</p>
              <p className="text-[#64748B] text-sm mt-1">Découvrez nos produits</p>
            </div>
            <button onClick={onClose} className="mt-2 bg-[#16A34A] text-white px-6 py-2.5 rounded-xl font-semibold text-sm hover:bg-[#15803d] transition-colors">
              Voir la boutique
            </button>
          </div>
        ) : (
                ...
                return (
                  <div key={item.id} className="flex gap-3 group">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/produit/${item.product.slug}`} onClick={onClose}
                        className="text-sm font-semibold text-[#0F172A] line-clamp-2 hover:text-[#16A34A] transition-colors leading-snug">
                        {item.product.name}
                      </Link>
                      {item.variant && (
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="text-right">
                          <p className="text-sm font-bold text-[#16A34A]">{formatPrice(unitPrice * item.quantity)}</p>
            ...
            <div className="border-t border-gray-100 px-5 pt-4 pb-6 shrink-0 space-y-3 bg-white">
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-[#64748B]">
                  ...
                  <span className="font-medium text-[#0F172A]">{formatPrice(subtotal)}</span>
                </div>
                <div className="flex justify-between text-[#64748B]">
                  <span className="flex items-center gap-1"><Truck size={12} />Livraison</span>
                  <span className={cn("font-medium", fee === 0 ? "text-[#16A34A]" : "text-[#0F172A]")}>{fee === 0 ? "Gratuite 🎉" : formatPrice(fee)}</span>
                </div>
              </div>
              <div className="flex justify-between items-center border-t border-gray-100 pt-3">
                <span className="font-bold text-[#0F172A]">Total</span>
                <span className="text-xl font-extrabold text-[#16A34A]">{formatPrice(total)}</span>
              </div>
              <Link href="/checkout" onClick={onClose} className="w-full flex items-center justify-center gap-2 bg-[#16A34A] hover:bg-[#15803d] text-white py-3.5 rounded-xl font-bold text-sm transition-colors">
                Passer la commande <ArrowRight size={16} />
              </Link>
              <Link href="/panier" onClick={onClose} className="w-full flex items-center justify-center gap-1 text-[#64748B] hover:text-[#0F172A] py-2 text-sm font-medium transition-colors">
